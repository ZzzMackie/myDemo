...
(function () {
  ...
  FunSpace._method('orientationDetect', function () {
    ...
    ._method('memoizer', function (memo, formula) {
      if (this._typeof(formula) !== 'function')
        throw new Error(formula + ' must be a function');
      if (this._typeof(memo) !== 'array')
        throw new Error(memo + ' must be a array');
    ...
    ._method('throttle', function (func, wait) {
      if (this._typeof(func) !== 'function')
        throw new Error(func + ' must be a function');
      if (this._typeof(wait) !== 'number')
        throw new Error(wait + ' must be a number');
      var timer = null;
      return function () {
    ...
    ._method('trim', function (str) {
      if (!str || this._typeof(str) !== 'string')
      ...
      for (var key in origin) {
        if (origin.hasOwnProperty(key)) {
          if (typeof origin[key] !== obj || origin[key] === null) {
            ...
            target[key] =
              toStr.call(origin[key]).slice(8, -1) === arrayStr ? [] : {};
    ...
    ._method('jsonDClone', function (obj) {
      if (!obj || this._typeof(obj) !== 'object')
    ...
    ._method('binary_search', function (low, high, key, arr) {
      if (
        this._typeof(low) !== 'number' ||
        this._typeof(high) !== 'number' ||
        this._typeof(key) !== 'number' ||
        this._typeof(arr) !== 'array'
      )
        throw new Error(
          low +
            ' ' +
    ...
    ._method('insertionSort', function (arr) {
      ...
      for (var right = 1; right < arr.length; right++) {
        //从第2位遍历到最后
        for (var left = right; arr[left] < arr[left - 1]; left--) {
          //比较当前位是否小于前一位
    ...
    ._method('randomNum', function (min, max) {
      if (
        this._typeof(min) !== 'number' ||
        this._typeof(max) !== 'number' ||
        max < min
      )
        throw new Error(
          min + ' ' + max + ' must be a number or ' + max + '<' + min
        );
    ...
    ._method('singleFun', function (fn) {
      if (!fn || this._typeof(fn) !== 'function')
        throw new Error(fn + ' must be a function');
      var result = null;
      return function () {
    ...
    ._method('scrollTo', function (x, y) {
      if (this._typeof(x) !== 'number' || this._typeof(y) !== 'number')
     ...
     * @instance
     * @function name - kthToLast
     * @param {ListNode} head
     * @param {number} k
     * @return {number}
     */
    ._method('kthToLast ', function (head, k) {
      let pre = head, // 前指针
        last = head, //  后指针
        pos = k;
      while (pos > 0) {
  ...
  Array.arrayEqual = function (arr, other) {
    if (
      FunSpace.prototype._typeof(other) !== 'array' ||
      FunSpace.prototype._typeof(arr) !== 'array'
    )
      throw new Error(arr + ' ' + other + ' must be a array');
  ...
  Array._dim = function (dimension, initial) {
    if (!dimension || FunSpace.prototype._typeof(dimension) !== 'number')
  ...
  Array._matrix = function (m, n, initial) {
    if (
      !m ||
      !n ||
      FunSpace.prototype._typeof(m) !== 'number' ||
      FunSpace.prototype._typeof(n) !== 'number'
    )
      throw new Error(m + ' ' + n + ' must be a number');
    var a,
      i,
      j,
      matArr = [];
